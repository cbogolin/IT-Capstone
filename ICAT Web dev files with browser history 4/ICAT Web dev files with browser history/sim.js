$(document).ready(function() {
  ...
  function sim (){
      results.empty();
      if (cv == "N/A"){
          alert("This country does not have a value for this policy. Please pick another one.");
          return
      }
      ...
      var gr = (nv - cv)/dt;
      var cgr = (1+(c*gr));
      var mi_1 = gdp*cgr; //first year
      miarray = [gdp, mi_1];

      var ll = c - 1.96 * stdErr;
      var ll_1 = gdp*(1+ll*gr);
      llarray = [gdp, ll_1];

      var ul = c + 1.96 * stdErr;
      var ul_1 = gdp*(1+ul*gr);
      ularray = [gdp, ul_1];
      
      results.append("Values Over Time" + '<br>');
      //equation stuff
      for (i = 2; i <= dt; i++)
      {
          miarray.push(miarray[i-1]*cgr); 
          llarray.push(llarray[i-1]*(1+ll*gr));
          ularray.push(ularray[i-1]*(1+ul*gr)); 
      }
  ...
  $.each(simCountries, function(key, value) {
  ...
  select.change(function(){
      ...
      var opval = $(this).val();
      countryName = opval;
      let countryObject = simCountries.find(country => country["Country Name"] === opval);
  ...
  policy.change(function(){
      ...
      let countryObject = simCountries.find(country => country["Country Name"] === countryName);
      ...
      currentDescription = json.policies[policyNum].description;
      stdErr = json.policies[policyNum].stdErr;
  ...
  function drawChart() {
      ...
      data.addColumn('number', 'Income Status Quo');
      data.addColumn('number', 'LL Income');
      data.addColumn('number', 'UL Income');
      deltaTime = dt; //years

      for (i = 0; i <= deltaTime; i++) {
          year = 2018 + i;
          yearString = '' + year;
          meanIncome = miarray[i];
          llIncome = llarray[i];
          ulIncome = ularray[i];
          data.addRow([yearString, meanIncome, gdp, llIncome, ulIncome]);
      }
      ...
      var options = {
          ...
          height: $("body").height() / 2,//$(".box").height,
          width: $(".col-md-9").width() * .9
      };
